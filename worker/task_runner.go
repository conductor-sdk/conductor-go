...
import (
	...
	metrics2 "github.com/conductor-sdk/conductor-go/metrics"
...
type TaskRunner struct {
	conductorTaskResourceClient *client2.TaskResourceApiService
...
func NewTaskRunner(authenticationSettings *settings2.AuthenticationSettings, httpSettings *settings2.HttpSettings) *TaskRunner {
...
func (c *TaskRunner) StartWorkerWithDomain(taskType string, executeFunction model2.ExecuteTaskFunction, threadCount int, pollInterval time.Duration, domain string) error {
...
//  - batchSize Amount of tasks to be polled. Each polled task will be executed and updated within its own unique goroutine.
//  - pollInterval Time to wait for between polls if there are no tasks available. Reduces excessive polling on the server when there is no work
func (c *TaskRunner) StartWorker(taskType string, executeFunction model2.ExecuteTaskFunction, batchSize int, pollInterval time.Duration) error {
...
func (c *TaskRunner) startWorker(taskType string, executeFunction model2.ExecuteTaskFunction, threadCount int, pollInterval time.Duration, taskDomain string) error {
...
func (c *TaskRunner) pollAndExecute(taskType string, executeFunction model2.ExecuteTaskFunction, pollInterval time.Duration, domain optional.String) error {
...
func (c *TaskRunner) runBatch(taskType string, executeFunction model2.ExecuteTaskFunction, pollInterval time.Duration, domain optional.String) (bool, error) {
...
func (c *TaskRunner) executeAndUpdateTask(taskType string, task model2.Task, executeFunction model2.ExecuteTaskFunction) error {
	defer concurrency.HandlePanicError("execute_and_update_task")
	taskResult, err := c.executeTask(&task, executeFunction)
	if err != nil {
		metrics2.IncrementTaskExecuteError(
			taskType, err,
		)
		return err
	}
...
func (c *TaskRunner) batchPoll(taskType string, count int, timeout time.Duration, domain optional.String) ([]model2.Task, error) {
	log.Debug(
		"Polling for task: ", taskType,
		", in batches of size: ", count,
	)
	metrics2.IncrementTaskPoll(taskType)
	startTime := time.Now()
	tasks, response, err := c.conductorTaskResourceClient.BatchPoll(
		context.Background(),
		taskType,
		&client2.TaskResourceApiBatchPollOpts{
			Domain:   domain,
	...
	metrics2.RecordTaskPollTime(
		taskType,
		spentTime.Seconds(),
	)
	if err != nil {
		metrics2.IncrementTaskPollError(
			taskType, err,
		)
		return nil, err
	}
...
func (c *TaskRunner) executeTask(t *model2.Task, executeFunction model2.ExecuteTaskFunction) (*model2.TaskResult, error) {
	log.Trace(
	...
	spentTime := time.Since(startTime)
	metrics2.RecordTaskExecuteTime(
		t.TaskDefName, float64(spentTime.Milliseconds()),
	)
	if err != nil {
		return nil, err
	}
	taskResult, err := model2.GetTaskResultFromTaskExecutionOutput(t, taskExecutionOutput)
	if err != nil {
		return nil, err
	}
	log.Trace(
...
func (c *TaskRunner) updateTaskWithRetry(taskType string, taskResult *model2.TaskResult) error {
	log.Debug(
	...
	for attempt := 0; attempt < taskUpdateRetryAttemptsLimit; attempt += 1 {
		...
		metrics2.IncrementTaskUpdateError(taskType, err)
		log.Debug(
...
func (c *TaskRunner) updateTask(taskType string, taskResult *model2.TaskResult) (*http.Response, error) {
	startTime := time.Now()
	...
	metrics2.RecordTaskUpdateTime(taskType, float64(spentTime))
	return response, err
}
