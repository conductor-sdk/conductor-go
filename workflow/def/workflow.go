...
import (
	...
	executor2 "github.com/conductor-sdk/conductor-go/workflow/executor"
	"net/http"
...
type ConductorWorkflow struct {
	executor         *executor2.WorkflowExecutor
	name             string
	version          int32
	description      string
	ownerEmail       string
...
func NewConductorWorkflow(executor *executor2.WorkflowExecutor) *ConductorWorkflow {
...
func (workflow *ConductorWorkflow) StartWorkflow(startWorkflowRequest *model2.StartWorkflowRequest) (workflowId string, err error) {
...
func (workflow *ConductorWorkflow) StartWorkflowsAndMonitorExecution(startWorkflowRequest *model2.StartWorkflowRequest) (executionChannel executor2.WorkflowExecutionChannel, err error) {
...
func (workflow *ConductorWorkflow) ToWorkflowDef() *model2.WorkflowDef {
	return &model2.WorkflowDef{
		Name:             workflow.name,
...
func getWorkflowTasksFromConductorWorkflow(workflow *ConductorWorkflow) []model2.WorkflowTask {
	workflowTasks := make([]model2.WorkflowTask, 0)
